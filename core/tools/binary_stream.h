...
class file_output_stream final : public stream {
public:
  ...
  int64_t all_size() const noexcept override { return bytes_writed_; }
  ...
  void swap(stream *_stream) noexcept override { im_assert(false); }
...
public:
  ...
  binary_stream(const binary_stream &_stream) { copy(_stream); }

  binary_stream(binary_stream &&_stream) noexcept { swap(_stream); }
  ...
  void reserve(int64_t _size) {
    if (buffer_.size() < static_cast<size_t>(_size))
      resize(_size);
  }

  void resize(int64_t _size) { buffer_.resize(_size); }

  void resize(int64_t _size, const char &_val) { buffer_.resize(_size, _val); }
  ...
  void reset_out() const noexcept { output_cursor_ = 0; }
  ...
  void write(const char *_data, int64_t _size) override {
    ...
    const auto size_need = input_cursor_ + _size;
    if (static_cast<size_t>(size_need) > buffer_.size())
      buffer_.resize(size_need + 1,
                     '\0'); // +1 it '\0' at the end of the buffer
  ...
  template <class t_> t_ read() const { return *((t_ *)read(sizeof(t_))); }

  bool save_2_file(const std::wstring &_file_name) const;

  static bool save_2_file(std::string_view _buf,
                          const std::wstring &_file_name);

  bool load_from_file(std::wstring_view _file_name);

  char *get_data_for_write() noexcept { return buffer_.data(); }
  ...
  void set_output(int64_t _value) noexcept { output_cursor_ = _value; }

  void set_input(int64_t _value) noexcept { input_cursor_ = _value; }
...
template <>
void core::tools::binary_stream::write<std::string>(const std::string &_val);
template <>
void core::tools::binary_stream::write<std::string_view>(
    const std::string_view &_val);

template <> std::string core::tools::binary_stream::read<std::string>() const;

inline constexpr size_t max_compress_bytes() noexcept { return 2'000'000'000U; }
inline constexpr size_t max_decompress_bytes() noexcept {
  return 1'000'000'000U;
}

bool compress_gzip(const char *_data, size_t _size,
                   binary_stream &_compressed_bs,
                   int _level = Z_BEST_COMPRESSION);
bool compress_gzip(const stream &_bs, binary_stream &_compressed_bs,
                   int _level = Z_BEST_COMPRESSION);
bool decompress_gzip(const char *_data, size_t _size,
                     binary_stream &_uncompressed_bs);
...
#ifndef STRIP_ZSTD
bool compress_zstd(const char *_data, size_t _size, std::string_view _dict,
                   binary_stream &_compressed_bs);
bool compress_zstd(const stream &_bs, std::string_view _dict,
                   binary_stream &_compressed_bs);
bool decompress_zstd(const char *_data, size_t _size, std::string_view _dict,
                     binary_stream &_uncompressed_bs);
bool decompress_zstd(const stream &_bs, std::string_view _dict,
                     binary_stream &_uncompressed_bs);
#endif // !STRIP_ZSTD
} // namespace tools
