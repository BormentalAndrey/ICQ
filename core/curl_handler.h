...
namespace core {
namespace curl_easy {
enum class completion_code { success = 0, cancelled, failed, resolve_failed };
...
class curl_task {
public:
  curl_task(std::weak_ptr<curl_context> _context,
            curl_easy::completion_function _completion_func);
  curl_task(std::weak_ptr<curl_context> _context,
            curl_easy::promise_t &&_promise);
...
public:
  ...
  virtual curl_easy::future_t
  perform(const std::shared_ptr<curl_context> &_context) = 0;
  virtual void
  perform_async(const std::shared_ptr<curl_context> &_context,
                curl_easy::completion_function _completion_func) = 0;
...
public:
  ...
  curl_easy::future_t
  perform(const std::shared_ptr<curl_context> &_context) override;
  void perform_async(const std::shared_ptr<curl_context> &_context,
                     curl_easy::completion_function _completion_func) override;
  ...
  void
  resolve_host(std::string_view _host,
               std::function<void(std::string _result, int _error)> _callback);
