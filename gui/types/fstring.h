...
namespace Data
{
    ...
    // Предварительное объявление FString и его внутренних типов
    class FString;
    struct SharedData; // Для FString::SharedData
    ...
    private:
        FStringView(std::shared_ptr<SharedData> _shared, int _offset, int _size);

        [[nodiscard]] core::data::range cutRangeToFitView(core::data::range _range) const;
        [[nodiscard]] core::data::format getFormat() const;
        std::shared_ptr<SharedData> sharedData() const;

    private:
        mutable std::weak_ptr<SharedData> data_;
        mutable int offset_;
        mutable int size_;
