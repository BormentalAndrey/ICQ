...
template <Qt::CaseSensitivity cs> struct QStringComparator {
  ...
  template <typename T> bool operator()(const char *lhs, const T &rhs) const {
    return operator()(QLatin1String(lhs), rhs);
  }

  template <typename T> bool operator()(const T &lhs, const char *rhs) const {
    return operator()(lhs, QLatin1String(rhs));
  }

  bool operator()(const char *lhs, const char *rhs) const {
    return operator()(QLatin1String(lhs), QLatin1String(rhs));
  }
};
